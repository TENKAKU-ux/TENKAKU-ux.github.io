...
function applyTheme(t, appearance) {
  ...
  document.body.className = (t.texture === 'flat' ? 'flat' : t.texture === 'washi' ? 'washi' : '') + (appearance === 'light' ? ' light' : '');
}
function App() {
  ...
  useEffect(() => {
    applyTheme(t, settings.appearance);
  }, [t.theme, t.accent, t.texture, settings.appearance]);
  ...
  function pick(m) {
    setMode(m);
    setRunSentences(window.CONTENT.pickRun(m));
    setAttempt(a => a + 1);
    setScreen('game');
  }
  function retry() {
    setRunSentences(window.CONTENT.pickRun(mode));
    setAttempt(a => a + 1);
    setScreen('game');
  }
  function finish(res) {
  ...
  function resetMistakes() {
    if (confirm('分析データ（直近の記録）を消去します。よろしいですか？')) {
      Store.resetAnalysis();
      setScreen('home');
      setTimeout(() => setScreen('analysis'), 0);
    }
  }
  return /*#__PURE__*/React.createElement("div", {
    ...
    onPick: pick
  }), screen === 'game' && /*#__PURE__*/React.createElement(Game, {
    key: mode.id + '-' + attempt,
    mode: mode,
  ...
  }), screen === 'analysis' && /*#__PURE__*/React.createElement(Analysis, {
